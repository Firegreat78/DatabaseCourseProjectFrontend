...
import { RefreshCw, Plus, AlertCircle, Save, Archive } from "lucide-react";
import {
  BarChart,
  Bar,
  XAxis,
...
const ExchangeAdminPage = () => {
  ...
  const handleArchiveStock = async (stockId) => {
    ...
    if (stock.is_archived) {
      alert("Эта ценная бумага уже архивирована. Разархивация недоступна.");
      return;
    }

    if (!window.confirm("Вы уверены, что хотите архивировать эту ценную бумагу? Архивные бумаги нельзя будет редактировать или разархивировать.")) {
      return;
    }
    ...
    try {
      ...
      fetchStocks();
      alert("Ценная бумага успешно архивирована!");
    } catch (error) {
